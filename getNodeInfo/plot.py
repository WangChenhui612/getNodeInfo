...
#获取分区名称
stdin5, stdout5, stderr5 = ssh.exec_command("lsblk | sed '/docker/d' | awk '{print $1}' ")
system_part_name = stdout5.read().decode('utf-8')
# print("机器分区:")
# print(system_part)

#获取分区大小
stdin6, stdout6, stderr6 = ssh.exec_command("lsblk | sed '/docker/d' | awk '{print $4}' ")
system_part_space = stdout6.read().decode('utf-8')
# print("机器分区:")
# print(system_part)

#判断是否为最小化安装
stdin7, stdout7, stderr7 = ssh.exec_command('cat /root/anaconda-ka.cfg')
system_part = stdout7.read().decode('utf-8')
...
# tb = pt.PrettyTable()
# tb.field_names = ["节点名称", "系统版本号", "GPU类型", "GPU卡个数", "CPU总核数", "分区名称", "分区大小", "是否为最小化安装"]
# tb.add_row(["10.166.15.243", centos_version, gpu_type, gpu_num, cpu_core_num, '12', '23', '是'])
# # tb.add_row(["Brifasdfae", 5905, 1857594, 1146.4])
# # tb.add_row(["Darwin", 112, 120900, 171423423423423424.7])
# # tb.add_row(["Hobart", 1357, 205556, 619.5])
# tb.reversesort = True

x = PrettyTable(["节点名称", "系统版本号", "GPU类型", "GPU卡个数", "CPU总核数", "分区名称", "分区大小", "是否为最小化安装"])
# x.align["City name"] = "l"# Left align city names
# x.padding_width = 1# One space between column edges and contents (default)
x.add_row(['10.166.15.243', centos_version, gpu_type, gpu_num, cpu_core_num, '12', '23', '是'])
